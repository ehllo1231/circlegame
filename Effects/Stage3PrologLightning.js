...
export class Stage3PrologLightning {
  ...
  configure(config = {}) {
    ...
    this.strokeWidth = Math.max(1, Number(cfg.strokeWidth) || 3);
    this.spawnDistance = Number.isFinite(cfg.spawnDistance) ? cfg.spawnDistance : 220;
    this.edgeOffset = Number.isFinite(cfg.edgeOffset) ? Math.max(0, cfg.edgeOffset) : 0;
    this.forkJitter = Math.max(0, Number(cfg.forkJitter) || 36);
